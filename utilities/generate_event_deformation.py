...
def distance_points_sphere(points, point, radius=6371.0):
    ...
    dlat = lat1 - lat2
    dlon = lon1 - lon2
    tmp = (numpy.sin(dlat/2.0)**2 + numpy.cos(lat1)*numpy.cos(lat2)*numpy.sin(dlon/2.0)**2)**0.5
...
def get_subfaults_in_envelope(centroid_strike_number, centroid_dip_number, 
    strike_numbers, dip_numbers, strike_envelope, dip_envelope):
    ...
    assert strike_envelope[0] <= 0, 'First entry of strike envelope must be <= 0'
    assert dip_envelope[0] <= 0, 'First entry of dip envelope must be <= 0'
    assert strike_envelope[1] >= 0, 'Second entry of strike envelope must be >= 0'
...
def find_subfaults_to_include(
    ...
    while residual_last > residual_now:
        ...
        for i in search_order:
            ...
            subfaults_temp = get_subfaults_in_envelope(centroid_strike_number,
                centroid_dip_number, along_strike_numbers, down_dip_numbers,
                strike_envelope_temp, dip_envelope)
        ...
        search_order = [prefer_down_dip_expansion, 1 - prefer_down_dip_expansion]
        for i in search_order:
            dip_envelope_temp = 1*dip_envelope
            dip_envelope_temp[i] += int(2.0*(i-0.5)) # -1 or +1
            subfaults_temp = get_subfaults_in_envelope(centroid_strike_number,
                centroid_dip_number, along_strike_numbers, down_dip_numbers,
                strike_envelope, dip_envelope_temp)
            new_resid = function_to_minimize(subfaults_temp)
            if new_resid < residual_now:
                dip_envelope = dip_envelope_temp
                residual_now = new_resid
   
        subfaults_included =  get_subfaults_in_envelope(centroid_strike_number,
            centroid_dip_number, along_strike_numbers, down_dip_numbers,
            strike_envelope, dip_envelope)
...
def create_event_new(source_zone, magnitude, 
    ...
    # some measure.
    subfaults_included = find_subfaults_to_include(along_strike_numbers,
        down_dip_numbers, centroid_index,
