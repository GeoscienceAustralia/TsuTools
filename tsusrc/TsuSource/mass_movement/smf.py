...
import TsuTools.utilities.log as lg
...
class Double_gaussian:
    ...
    def __call__(self, x, y):
        ...
        # If x and y are 2D arrays (i.e. a grid), reshape
        # for calculations below. First get shape info
        shapex = num.shape(x)
        shapey = num.shape(y)
        if len(shapex) > 1:
            x = x.flatten()
        ...
        # If input x and y were a 2D grid, reshape z before returning
        if len(shapex) > 1:
            z = num.reshape(z, shapex)
        return z
