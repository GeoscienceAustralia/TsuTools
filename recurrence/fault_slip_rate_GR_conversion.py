...
def slip2GR(slip_rate, A, b, M_max, M_min=0, mu=3e11):
    ...
    # Calculate a
    a = (mu*A*(slip_fault)*(c-b)*(1-np.exp(-1*beta*(M_max-M_min))))/(b*moment_max*np.exp(-1*beta*(M_max-M_min)))
...
def GR2sliprate(a, b, A, M_max, M_min=0, mu=3e11):
    ...
    slip_rate = (b*lambda_min*moment_max*np.exp(-1*beta*(M_max-M_min)))/(mu*A*(c-b)*(1-np.exp(-1*beta*(M_max-M_min))))
    slip_rate = slip_rate*10 # Convert to mm/a
    moment_rate = (b*lambda_min*moment_max*np.exp(-1*beta*(M_max-M_min)))/((c-b)*(1-np.exp(-1*beta*(M_max-M_min))))
